...
# ------------------------------------------------------------------------------
# Health Endpoint -> check if API is working
# ------------------------------------------------------------------------------
...
# dragging db functionality into this function using Depends
# makes DB available inside function
@app.post(
    "/auth/register", 
    response_model=UserResponse, # return this with status code
    status_code=status.HTTP_201_CREATED,
    tags=["auth"]
)
def register(user_create: UserCreate, db: Session = Depends(get_db)):
    # Exclude confirm_password before passing data to User.register
    # this is how we are validating data that is coming in
    user_data = user_create.dict(exclude={"confirm_password"})
    try: #EAFP
        user = User.register(db, user_data)
        db.commit()

        # if we dont do refresh here, we get stale data in the user
        # basically commiting it, then getting user updates for user model that have been stored after commit
        db.refresh(user)
        return user
...
def create_calculation(
    ...
    try:
        ...
        # usually you dont want to store result of the calculation in the record
        # reason we dont want to do this is because the result could get stale if the values change
...
def list_calculations(
    ...
    # getting all calculations for currently logged in user (person sending this request)
